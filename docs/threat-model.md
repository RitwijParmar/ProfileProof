...
Assets include LinkedIn session secrets, professional-profile data, service availability, Cloud Run identity,
and the integrity of provenance. Trust boundaries are public internet to Cloud
Run, Cloud Run to the container, the container to fixed LinkedIn endpoints,
...
| Incorrect identity match | The normalized response root URN and returned public identifier must match the requested identifier | Upstream source records can still be stale or wrong |
| Excessive personal-data collection | The fixed full-profile request is mapped only to named professional fields; the API omits contact, email, phone, address, and unrelated social fields | Requested summaries may contain free-form personal information |
| Secret or token leakage | Session values come from Secret Manager and are never returned; API keys are stored as digests | Platform configuration and privileged operators remain trusted |
...
| Cache privacy | Profile records are cached only in process memory for one hour; no database or disk persistence | Instance-memory inspection by a privileged operator is out of scope |
